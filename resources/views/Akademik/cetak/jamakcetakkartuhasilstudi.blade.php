                    ...
                    <table border='1' rules='all0' width='100%'
                        ...
                        <tfoot>
                            ...
                            <tr>
                                ...
                                <td style='width:20px;padding-left:5px;text-align:center;'><span id="ipk_kumulatif{{ $row }}"></span></td>
...
@foreach ($pecah as $row)
    <script>
        $(document).ready(function () {
            ...
            document.getElementById("tanggalwaktu{{ $row }}").innerHTML = tanggal + " " + bulanarray[bulan] + " " + tahun;

            var nim = $('#nim{{ $row }}').val();
            var tahun = $('#tahun{{ $row }}').val();
            var semester = $('#semester{{ $row }}').val();

            // 1. Ambil data KHS semester ini (IPS)
            $.ajax({
                type: 'POST',
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < jml; i++) {
                        var nilaihuruf = data[i].nilai_akhir_huruf || "";
                        var mutu = data[i].mutu || 0;
                        var bobot = data[i].sks_matakuliah * mutu;

                        tampil += '<tr style="font-size:9px;"><td style="text-align:center;">' +
                    ...
                    if (totalipk >= 3.00) batassks = 24;
                    else if (totalipk >= 2.50) batassks = 21;
                    else if (totalipk >= 2.00) batassks = 18;
                    else if (totalipk >= 1.50) batassks = 15;
                    else batassks = 12;

                    $('#kartuhasilstudi{{ $row }}').html(tampil);
                    $('#totalsks{{ $row }}').html(totalsks);
                    $('#totalbobot{{ $row }}').html(totalbobot.toFixed(2));
                    $('#batassks{{ $row }}').html("Diperbolehkan mengambil SKS max. " + batassks);
                    $('#totalipk{{ $row }}').html(totalipk.toFixed(2));

                    // 2. Ambil IPK Kumulatif
                    $.ajax({
                        type: 'POST',
                        dataType: "json",
                        url: "{{ config('setting.second_url') }}akademik/get-seluruh-khs1",
                        headers: {
                            "Authorization": 'Bearer ' + token,
                            "username": userlogin
                        },
                        ...
                        success: function (res) {
                            ...
                            for (var j = 0; j < res.length; j++) {
                                var sks = parseFloat(res[j].sks_matakuliah);
                                var mutu = parseFloat(res[j].mutu);
                                if (!isNaN(sks) && !isNaN(mutu)) {
                                    totalSksAll += sks;
                                    totalBobotAll += sks * mutu;
                                }
                            }

                            var ipk = totalBobotAll / totalSksAll;
                            console.log(ipk);
                            $('#ipk_kumulatif{{ $row }}').html(ipk.toFixed(2));
                        }
                    });
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            $('.namaprodi_{{ $row }}').html(result.nama_program_studi);
                            $('.namamhs_{{ $row }}').html(result.nama_mahasiswa);
                            $('#kaprodi{{ $row }}').html(result.namaprodi);
                            $('#dosenwali{{ $row }}').html(result.dosen_wali);

                            var kdjnjang = (result.kode_jenjang_pendidikan == '1') ? "D3" : "S1";
                            $('.jenjange{{ $row }}').html(kdjnjang);
                            $('#dekanebos{{ $row }}').html(result.dekane);
                            $('#qrdekan{{ $row }}').html(
                                `<img src="{{ url('qrcodedosenmanajemen/` + result.nidndekane + `_Kartu_Ujian_Akhir_Semester.png') }}" alt="Image" width='60' height='60' id="qrImage{{ $row }}"><br> <small><b>valid_id : ` + result.valididdekane + `</b></small>`
                            );
                            $('#nama_fakultas{{ $row }}').html('FAKULTAS ' + result.nama_fakultas.toUpperCase());
...
<!--                            $('#dekanebos{{ $row }}').html(result.dekane);-->
<!--                            $('#qrdekan{{ $row }}').html(-->
<!--                                `<img src="{{ url('qrcodedosenmanajemen/` + result.nidndekane + `_Kartu_Ujian_Akhir_Semester.png') }}" alt="Image" width='60' height='60' id="qrImage{{ $row }}"><br> <small><b>valid_id : ` +-->
<!--                                result.valididdekane + `</b></small>`-->
<!--                            );-->
