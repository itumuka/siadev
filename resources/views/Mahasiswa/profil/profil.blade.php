    ...
    <div class="row">
        <div class="col-12 col-lg-5 col-xl-4">
            ...
            <div class="widget-user-image">
                <img id="foto_profil_view" class="rounded-circle" src="{{ url('images/nouser.gif') }}" alt="User Avatar" style="width: 90px; height: 90px; object-fit: cover;">
            </div>
...
// tampilkabupatenibu();
            ...
            success: function(result) {
                console.log(result);
                    ...
                    if (result.foto && result.foto !== "") {
                        // Menggunakan config setting.second_url namun membersihkan '/api/' di ujungnya
                        var api_base_url = "{{ config('setting.second_url') }}".replace(/\/api\/$/, "");
                        var imagePath = api_base_url + "/images/foto_mahasiswa/" + result.foto;
                        $('#foto_profil_view').attr("src", imagePath);
                    }
                    ...
                    // Sequence loading to avoid race conditions and trigger conflicts
                    is_loading_profil = true;
                    $('#kode_provinsi').val(result.kode_provinsi).trigger('change.select2');
    
                    loadKabupaten(result.kode_provinsi).then(function() {
                        $('#kode_kabupaten').val(result.kode_kabupaten).trigger('change.select2');
                        return loadKecamatan(result.kode_provinsi, result.kode_kabupaten);
                    }).then(function() {
                        $('#kode_kecamatan').val(result.kode_kecamatan).trigger('change.select2');
                        is_loading_profil = false;
                    });
    ...
    $('#kode_provinsi').change(function() {
        if (is_loading_profil) return;
        var kd_provinsi = $(this).val();
        if (kd_provinsi) {
            loadKabupaten(kd_provinsi).then(function() {
                if (!is_loading_profil) {
                    // Reset kecamatan setelah kabupaten dimuat hanya jika tidak sedang loading data profil awal
