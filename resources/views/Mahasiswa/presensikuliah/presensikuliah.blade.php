                        ...
                        render: function(data, type, row, meta) {
                            ...
                            if (row.button_in > 0) {
                                if (row.kehadiran != null) {
                                    disable =
                                        '<span class="waves-effect waves-light btn btn-sm btn-outline btn-primary mb-5"><i class="fa fa-check"></i> Hadir</span>'
                                } else {
                                    if (row.cekuts == "0" && parseInt(row.pertemuan_ke) >= 20) {
                                        disable =
                                            '<span class="mb-5 text-danger">Block UTS <br> <small>Belum Bayar atau Belum ACC Keuangan</small></span>';
                                    } else if (row.cekuas == "0" && parseInt(row.pertemuan_ke) >=
                                        20) {
                                        disable =
                                            '<span class="mb-5 text-danger">Block UAS</span>';
                                    } else {
                                        disable =
                                            '<button type="button" class="waves-effect waves-light btn btn-sm btn-outline btn-success mb-5" id="clock_in" data-id_kelas="' +
                                ...
                                disable = '<button class="waves-effect waves-light btn btn-sm btn-outline btn-default mb-5"><i class="fa fa-lock"></i> </button>';
                            }
                            if (row.url_rps != null || row.url_rps != "") {
                                bt_rps = `<a href="` + row.url_rps + `" target="_blank" class="waves-effect waves-light btn btn-sm btn-outline btn-primary" title="Lihat RPS"><i class="fa fa-file-pdf-o"></i></a>`;
                            }else{
                                bt_rps = `<a href="javascript:void(0)" class="waves-effect waves-light btn btn-sm btn-outline btn-default" title="Lihat RPS"><i class="fa fa-file-pdf-o"></i></a>`;
                            }
                            

                            
                            return disable+' '+bt_rps;
                        }
                    },
                    {
                        data: null,
