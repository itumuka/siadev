                ...
                <div class="sembunyi" id="warning-content">
                    <div class="box box-solid bg-warning">
                        ...
                        <div class="box-body text-danger">
                            Anda belum mengisi kuesioner di <a href="javascript:void(0);" id="siedomButton" class="btn btn-xs btn-danger">SIEDOM</a>. Silahkan klik button SIEDOM dan isi kuesioner setiap matakuliah terlebih dahulu.
...
$(document).ready(function() {
    ...
    function table_khs(id = 0) {
        var table = $("#tbkhs").DataTable({
            ...
            columns: [
                ...
                { data: 'nilai_akhir_huruf', className: "text-center" },
    ...
    function redirectToSiedom() {
        // Ambil token JWT dari localStorage
        var token = localStorage.getItem("user_token");

        // Periksa apakah token ada
        if (!token) {
            alert("Token tidak ditemukan. Harap login di SIAKAD terlebih dahulu.");
            return;
        }

        // Kirim token JWT ke SIEDOM
        $.ajax({
            type: 'POST',
            headers: {
                "Authorization": 'Bearer ' + token, // Lampirkan token JWT di header Authorization
                "X-CSRF-TOKEN": '{{ csrf_token() }}'
            },
            url: "{{ env('SIEDOM_URL') }}auth/verify-token", // Endpoint verifikasi token di SIEDOM
            success: function (response) {
                // Jika verifikasi berhasil
                if (response.status === 'Token valid') {
                    // Redirect ke dashboard SIEDOM di tab baru
                    window.open("{{ env('SIEDOM_URL') }}dashboard", '_blank');
                } else {
                    // Token valid, tetapi ada masalah lain (opsional, jika ingin cek pesan error)
                    // console.error(response.message);
                    alert("Token tidak valid atau Anda tidak memiliki akses.");
                }
            },
            error: function (xhr) {
                // Jika gagal (e.g., token expired atau tidak valid)
                console.error(xhr.responseText);
                alert("Gagal login ke SIEDOM. Token tidak valid atau expired.");
            }
        });
    }
    ...
    // Tambahkan listener pada tombol "Siedom"
    $('#siedomButton').on('click', function() {
        redirectToSiedom();
    });
