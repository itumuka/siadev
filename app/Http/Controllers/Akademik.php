...
class Akademik extends Controller
{
    ...
    public function qrcode()
    {
        ...
        $title = "Akademik SIAKAD UMUKA";
        return view('Akademik/master/qrcode', compact('title'));
    }
    public function qrcodemanajemen()
    {
        $title = "Akademik SIAKAD UMUKA";
        return view('Akademik/master/qrcodemanajemen', compact('title'));
    }
    ...
    public function presensi_mhs()
    {
        ...
        $child_breadcrumb = "";
        return view('Akademik/presensi_mhs/presensi_mhs', compact('title', 'parent_breadcrumb', 'child_breadcrumb', 'session_tahun', 'session_semester', 'session_kode_program_studi', 'session_tipe', 'session_nama_tahunakademik'));
    }
    ...
    public function jadwalujian()
    {
        ...
        $title = "Jadwal Ujian";
        $parent_breadcrumb = "Input Jadwal Ujian";
        $child_breadcrumb = "Jadwal Ujian";
        return view('Akademik/matakuliah/jadwalujian', compact('title', 'parent_breadcrumb', 'child_breadcrumb', 'session_tahun', 'session_semester', 'session_kode_fakultas', 'session_jabatan', 'session_nama_tahunakademik'));
    }
    ...
    public function cetakkhs($a, $b, $c, $d, $e)
    {
        $nim = $a;
        $tahun = $b;
        $semester = $c;
        ...
        $title = "Cetak KHS";
        return view('Akademik/cetak/cetakkhs_mhs', compact('title', 'nim', 'semester', 'tahun', 'session_nama_tahunakademik', 'tgl', 'id_her', 'kode_nilai'));
    }
    ...
    public function jamakcetakdaftarhadirujian($id_tawar,$jenisujian)
    {
        $id_tawar = $id_tawar;
        $jenisujian = $jenisujian;

        $title = "Cetak Daftar Hadir Ujian";
        return view('Akademik/cetak/jamakcetakdaftarhadirujian', compact('title', 'id_tawar','jenisujian'));
    }
    public function jamakcetakkartuujian($nim, $tahun, $semester, $jenisujian)
    {
        $nim = $nim;
        $tahun = $tahun;
        $semester = $semester;
	$jenisujian= $jenisujian;
        ...
        $title = "KARTU UJIAN";
        return view('Akademik/cetak/jamakcetakkartuujian', compact('title', 'nim', 'tahun', 'semester','jenisujian'));
    }
    ...
    public function jamakcetakkartuhasilstudi($nim, $tahun, $semester, $kodenilai)
    {
        ...
        $title = "KARTU RENCANA STUDI";
        return view('Akademik/cetak/jamakcetakkartuhasilstudi', compact('title', 'nim', 'tahun', 'semester','kode_nilai'));
    }
    ...
    public function cetakberitaacaraskripsi($id)
    {
        $id_skripsi_ujian = $id;
        ...
        $title = "Cetak Berita Acara Ujian Skripsi";
    ...
    public function lihat_penilaian()
    {
        ...
        $title = "Lihat Mata Kuliah Seluruh Prodi";
        $parent_breadcrumb = "Penilaian Mata Kuliah";
        $child_breadcrumb = "Progres Penilaian";
        return view('Akademik/lihat_penilaian/index', compact('title', 'parent_breadcrumb', 'child_breadcrumb', 'session_tahun', 'session_semester', 'session_tipe'));
    }
    public function saveAllQrCode(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            // Tentukan path folder tujuan di dalam folder public
            $destinationPath = public_path('qrcodedosen');
            ...
            // Pindahkan file ke folder public/qrcodedosen
            $file->move($destinationPath, $fileName);

            // Mengembalikan response dengan path relatif file yang disimpan
            return response()->json([
                'success' => 'QR Code berhasil disimpan!',
                'path' => asset('qrcodedosen/' . $fileName)
            ]);
        }
    ...
    public function saveAllQrCodeManajemen(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            // Tentukan path folder tujuan di dalam folder public
            $destinationPath = public_path('qrcodedosenmanajemen');
            ...
            return response()->json([
                'success' => 'QR Code berhasil disimpan!',
                'path' => asset('qrcodedosenmanajemen/' . $fileName)
            ]);
        }
    ...
    public function saveAllQrCodeACC(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            $destinationPath = public_path('qrcodeaccdowal');
    
            // 🔍 Kalau folder tujuan adalah symlink, resolve ke path asli
            if (is_link($destinationPath)) {
                $realPath = readlink($destinationPath);
                if ($realPath !== false) {
            ...
            return response()->json([
                'success' => 'QR Code berhasil disimpan!',
                'path' => asset('qrcodeaccdowal/' . $fileName)
            ]);
        }
